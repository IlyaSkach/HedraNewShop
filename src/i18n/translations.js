export const translations = {
  en: {
    // Header
    phone: "Call",
    whatsapp: "WhatsApp",
    telegram: "Telegram",
    vk: "VKontakte",
    cart: "Cart",

    // Banner
    bannerTitle: "Welcome to HedraNewShop",
    bannerSubtitle: "Quality beverages with delivery",

    // Categories
    categoriesTitle: "Categories",
    beer: "Beer",
    wine: "Wine",
    spirits: "Spirits",
    rtd: "RTD",
    comingSoon: "Coming Soon",

    // Subcategories
    beerSubcategoriesTitle: "Beer - Choose Brand",
    wineSubcategoriesTitle: "Wine - Choose Type",
    spiritsSubcategoriesTitle: "Spirits - Choose Type",
    rtdSubcategoriesTitle: "RTD - Choose Brand",

    // Products
    productsTitle: "Products",
    alcohol: "Alcohol",
    addToCart: "Add to Cart",

    // Cart
    yourCart: "Your Cart",
    cartEmpty: "Your cart is empty",
    quantity: "Quantity",
    subtotal: "Subtotal",
    delivery: "Delivery",
    total: "Total",
    deliveryArea: "Delivery area",
    selectArea: "Select delivery area",
    hurghada: "Hurghada",
    alAhyaa: "Al Ahyaa",
    orderWhatsApp: "Order via WhatsApp",
    remove: "Remove",

    // WhatsApp Message
    newOrder: "New order from HedraNewShop",
    orderTotal: "Total",

    // Currency
    currency: "EGP",
  },
  ru: {
    // Header
    phone: "Позвонить",
    whatsapp: "WhatsApp",
    telegram: "Telegram",
    vk: "ВКонтакте",
    cart: "Корзина",

    // Banner
    bannerTitle: "Добро пожаловать в HedraNewShop",
    bannerSubtitle: "Качественные напитки с доставкой",

    // Categories
    categoriesTitle: "Категории",
    beer: "Пиво",
    wine: "Вино",
    spirits: "Крепкие напитки",
    rtd: "RTD",
    comingSoon: "Скоро",

    // Subcategories
    beerSubcategoriesTitle: "Пиво - Выберите бренд",
    wineSubcategoriesTitle: "Вино - Выберите тип",
    spiritsSubcategoriesTitle: "Крепкие напитки - Выберите тип",
    rtdSubcategoriesTitle: "RTD - Выберите бренд",

    // Products
    productsTitle: "Товары",
    alcohol: "Алкоголь",
    addToCart: "В корзину",

    // Cart
    yourCart: "Ваша корзина",
    cartEmpty: "Ваша корзина пуста",
    quantity: "Количество",
    subtotal: "Сумма товаров",
    delivery: "Доставка",
    total: "Итого",
    deliveryArea: "Зона доставки",
    selectArea: "Выберите зону доставки",
    hurghada: "Хургада",
    alAhyaa: "Эль Ахьяа",
    orderWhatsApp: "Заказать через WhatsApp",
    remove: "Удалить",

    // WhatsApp Message
    newOrder: "Новый заказ из HedraNewShop",
    orderTotal: "Итого",

    // Currency
    currency: "EGP",
  },
};
